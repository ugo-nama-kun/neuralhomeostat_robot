...
int read_servo_i = 1;
//int servo_volts[] = {0,0,0,0, 0,0,0,0};
...
void read_servo_positions()
{
  ...
  bool result = false;
    ...
    */
    ...
    // volatages, read one servo per cycle to speed up cycle time
    /*
    {
      int8_t get_data = 0;
  
      dxl_comm_result = packetHandler->read1ByteTxRx(portHandler, read_servo_i, ADDR_AX_PRESENT_VOLTAGE, (uint8_t*)&get_data, &dxl_error);
      if (dxl_comm_result != COMM_SUCCESS)
      {
        Serial.println("- Failed to get present voltage");
        volt_failures |= 1<<(read_servo_i-1);
      } else {
        servo_volts[read_servo_i-1] = get_data;
      }
          
      for (int i = 1; i <= 8; i++) {
        Serial.print(servo_volts[i-1]);
        Serial.print("\t");
      }
    }
    */
    ...
    if (volt_failures > 0)
    {
        Serial.print("- Failed to get present voltage 0x");
        Serial.println(volt_failures, HEX);  
    } 
    */   
}
...
void setup() {
    ...
    for (int i = 1; i <= 8; i++) {
        ...
        //dxl_comm_result = packetHandler->write2ByteTxRx(portHandler, i, ADDR_AX_TORQUE_LIMIT, 256, &dxl_error);
        if (dxl_comm_result != COMM_SUCCESS)
        {
