...
from playroom_env.envs.playroom_env import PlayroomEnv
from playroom_env.envs.navigation_env import NavigationEnv
...
class RealAntEnv(MujocoEnv, utils.EzPickle):
    ...
    def update_xml(self, tree, is_domain_rand: bool, no_wall=True, activity_range=0.75):
        ...
        if is_domain_rand:
            ...
            # imu_elem.set("pos", f"{random.uniform(-0.03, 0.03)} {random.uniform(-0.03, 0.03)} {0.14 + random.uniform(-0.03, 0.03)}")
            # imu_elem.set("euler", f"{random.uniform(-2, 2)} {180 + random.uniform(-2, 2)} {random.uniform(-2, 2)}")
            ...
            scale = 0.09
            x, y, z = self.generate_random_point_in_unit_sphere()
            ...
            # tmp = tree.find(".//geom[@name='random_weight']").get("mass")
            # print(f"body weights: {base_elem.get('mass')} + {tmp} = {new_body_weight + attached_weight}")
        else:
            asset = tree.find(".//asset")
            ET.SubElement(
                asset, "hfield", dict(
    ...
    def generate_random_point_in_unit_sphere(self):
    ...
    def get_image(
        ...
        if mode == "rgbd_array":
            viewers = [self.mujoco_renderer._get_viewer(render_mode="rgb_array")]
            viewers.append(self.mujoco_renderer._get_viewer(render_mode="depth_array"))
        else:
            viewers = [self.mujoco_renderer._get_viewer(render_mode=mode)]
        ...
        if mode == "rgbd_array":
            im = self.mujoco_renderer.render(
                ...
                camera_id,
                camera_name, )
            im_d = self.mujoco_renderer.render(
                "depth_array",
                camera_id,
...
class RealAntOnTheBoardEnv(OnTheBoardEnv):
    MODEL_CLASS = RealAntEnv
    ORI_IND = 3
...
class RealAntCommandEnv(CommandEnv):
    MODEL_CLASS = RealAntEnv
    ORI_IND = 3
