...
# Action of the cooling behavior
FLAT_POSTURE_ACTION = np.array([0, -1, 0, 1, 0, 1, 0, -1], dtype=np.float32)

# some big value for logarithm of δ(a; a_flat) at a_flat
LOGPROB_DIRAC_DELTA = 100.0
...
class CategoricalHead(nn.Module):
    ...
    def forward(self, logits):
        return torch.distributions.Independent(
            torch.distributions.Categorical(logits=logits), 1
        )
...
class GaussianHead(nn.Module):
    ...
    def forward(self, x):
        return torch.distributions.Normal(x, self.scale)
...
def test_env_cooling_behavior(agent: torch.nn.Module, test_envs: gym.wrappers.RecordEpisodeStatistics, device, render=False):
    agent.eval()
    ...
    while np.any(list(not_done_flags.values())):
        ...
        obs, reward, done, truncated, info = test_envs.step(motor_action)
        done = done | truncated
